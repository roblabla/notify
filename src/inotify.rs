...
struct EventLoop {
    ...
    event_loop_tx: mpsc::Sender<EventLoopMsg>,
    event_loop_rx: mpsc::Receiver<EventLoopMsg>,
...
pub struct INotifyWatcher {
    channel: Mutex<mpsc::Sender<EventLoopMsg>>,
    waker: Arc<mio::Waker>,
}
...
impl EventLoop {
    pub fn new(inotify: Inotify, event_tx: EventTx) -> Result<EventLoop> {
        let (event_loop_tx, event_loop_rx) = mpsc::channel::<EventLoopMsg>();
...
impl Watcher for INotifyWatcher {
    fn new_raw(tx: Sender<RawEvent>) -> Result<INotifyWatcher> {
        ...
        let channel = Mutex::new(event_loop.event_loop_tx.clone());
    ...
    fn watch<P: AsRef<Path>>(&mut self, path: P, recursive_mode: RecursiveMode) -> Result<()> {
        ...
        self.channel.lock().unwrap().send(msg).unwrap();
...
impl Drop for INotifyWatcher {
    fn drop(&mut self) {
        ...
        self.channel.lock().unwrap().send(EventLoopMsg::Shutdown).unwrap();
        self.waker.wake().unwrap();
    }
}
